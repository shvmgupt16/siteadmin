			 ...
			 //$filename='';
			  //if (is_uploaded_file($_FILES['image']['tmp_name']))
				//	{
					...
					//$r = uniqid();					
					//$filename= $r.'_'.$image;
					//move_uploaded_file($_FILES['image']['tmp_name'], "../uploads/news/".$filename);	
					//}
			  
			  
			  $q = "insert into ".$this->_table." (service,description,status) values ('".$_POST['service']."','".addslashes($_POST['description'])."','Y')";	
			if(mysql_query($q))
			{
			 ...
			 // if (is_uploaded_file($_FILES['image']['tmp_name']))
				 //	{
					 ...
					 //{
						 //$filename = $_POST['img'];
				 //	}
			$q = "update ".$this->_table." set service = '".$_POST['service']."',description = '".addslashes($_POST['description'])."',status='Y' where id=".$_POST['id'];
			if(mysql_query($q))
			{
